...
def extract_weather_batch():
    ...
    url_template = "http://api.weatherapi.com/v1/current.json?key={key}&q={city}&aqi=no"

    results = []
    for city in cities:
        url = url_template.format(key=api_key, city=city)
...
with DAG(
    ...
    target_table = 'GROUP6_DB.RAW.WEATHER_TOP10_CA'
